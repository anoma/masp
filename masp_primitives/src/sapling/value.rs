//! Monetary values within the MASP shielded pool.
//!
//! Values are represented in three places within the MASP protocol:
...
//!   single [`Bundle`].
//! - `valueBalanceSapling`, which is a signed 127-bit integer. This is represented
//!   by a user-defined type parameter on [`Bundle`], returned by
//!   [`Bundle::value_balance`] and [`SaplingBuilder::value_balance`].
//!
//! If your specific instantiation of the MASP protocol requires a smaller bound on
...
//!   be checked in its `TryFrom<i128>` implementation.
...
impl ValueCommitment {
    ...
    pub fn derive(
        value_commitment_generator: SubgroupPoint,
        value: NoteValue,
        rcv: ValueCommitTrapdoor,
    ) -> Self {
        let cv = (value_commitment_generator * jubjub::Scalar::from(value.0))
...
mod tests {
    ...
    proptest! {
        ...
        fn bsk_consistent_with_bvk(
            ...
            let bvk = values
                .into_iter()
                .map(|(value, rcv)| ValueCommitment::derive(asset_type, value, rcv))
                .sum::<CommitmentSum>()
