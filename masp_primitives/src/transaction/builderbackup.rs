...
impl<R: RngCore+CryptoRng> SaplingBuilder<R> {
...
pub mod testing {
    ...
    fn fails_on_negative_change() {
        ...
        // 0.0001 t-ZEC fee
        {
            ...
            assert_eq!(
                ...
                Err(builderbackup::Error::ChangeIsNegative(Amount::from_pair(zec(), -10000).unwrap()))
            );
        } 
        ...
        // 0.0005 z-ZEC out, 0.0001 t-ZEC fee
        {
            ...
            assert_eq!(
                builder.build(consensus::BranchId::Sapling, &MockTxProver),
                Err(builderbackup::Error::ChangeIsNegative(Amount::from_pair(zec(), -60000).unwrap()))
            );
        }
        ...
        builder
            .add_spend(rng,
                ...
                *to.diversifier(),
                ...
                .unwrap();
            builder
                .add_spend(rng, extsk, *to.diversifier(), note2, witness2.path().unwrap())
                .unwrap();
            builder
                .add_output(rng, ovk, to, zec(), 30000, None)
                .unwrap();
            builder
                .add_transparent_output(
                    &TransparentAddress::PublicKey([0; 20]),
                ...
                .unwrap();
            assert_eq!(
                builder.build(crate::consensus::BranchId::Sapling, &MockTxProver),
                Err(builderbackup::Error::BindingSig)
            )
        }
    }
    ...
    prop_compose! {
        fn arb_bundle()(n_notes in 1..30usize)(
            ...
            let mut builder = SaplingBuilder::new(TEST_NETWORK);
