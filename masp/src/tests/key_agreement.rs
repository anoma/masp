...
use masp_primitives::primitives::{Diversifier, ViewingKey};
use rand_core::{OsRng, RngCore};

use crate::{
    libmasp_sapling_generate_r, libmasp_sapling_ka_agree, libmasp_sapling_ka_derivepublic,
};
...
fn test_key_agreement() {
    ...
    // Create random esk
    let mut esk = [0u8; 32];
    libmasp_sapling_generate_r(&mut esk);
    ...
    // Serialize pk_d for the call to libmasp_sapling_ka_agree
    let addr_pk_d = addr.pk_d().to_bytes();

    assert!(libmasp_sapling_ka_agree(
        &addr_pk_d,
        &esk,
    ...
    let mut epk = [0u8; 32];
    assert!(libmasp_sapling_ka_derivepublic(
        &addr.diversifier().0,
        &esk,
        &mut epk
    ));
