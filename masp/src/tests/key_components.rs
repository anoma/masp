...
fn key_components() {
    ...
    for tv in test_vectors {
        ...
        {
            let mut ak = [0u8; 32];
            libmasp_ask_to_ak(&tv.ask, &mut ak);
            assert_eq!(&ak, &tv.ak);
        }
        ...
        {
            let mut nk = [0u8; 32];
            libmasp_nsk_to_nk(&tv.nsk, &mut nk);
            assert_eq!(&nk, &tv.nk);
        }
        ...
        assert!(libmasp_check_diversifier(&tv.default_d));
        ...
        assert_eq!(&addr.pk_d().to_bytes(), &tv.default_pk_d);
        {
            ...
            libmasp_ivk_to_pkd(&tv.ivk, &tv.default_d, &mut default_pk_d);
        ...
        let note = addr
            .create_note(asset_type, tv.note_v, Rseed::BeforeZip212(note_r))
            .unwrap();
