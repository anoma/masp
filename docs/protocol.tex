...
\newcommand{\convertStatement}{\term{Convert statement}}
\newcommand{\ConvertStatement}{\titleterm{Convert Statement}}
...
\newcommand{\NotePositionTypeConvert}{\binaryrange{\MerkleDepthConvert}}
...
\newcommand{\ValueCommitConvert}{\ValueCommitAlg\mathsf{.Convert}}
...
\newcommand{\ValueConvert}[1]{\Value^\mathsf{convert}_{#1}}
...
\newcommand{\ValueCommitRandConvert}[1]{\ValueCommitRand^\mathsf{convert}_{#1}}
...
\newcommand{\AllowedConversionValueBase}{\mathsf{vb}}
...
\newcommand{\cvAllowedConversion}[1]{\cv^\mathsf{AllowedConversion}_{#1}}
\newcommand{\cm}{\mathsf{cm}}
\newcommand{\cmU}{\cm_{\kern -0.06em u}}
\newcommand{\cmOld}[1]{\cm^\mathsf{old}_{#1}}
\newcommand{\cmNew}[1]{\cm^\mathsf{new}_{#1}}
\newcommand{\cmAllowedConversion}[1]{\cm^\mathsf{convert}_{#1}}
...
\newcommand{\MerkleDepthConvert}{\MerkleDepth^\mathsf{Convert}}
...
\newcommand{\MerkleHashLengthConvert}{\mathsf{\ell_{MerkleConvert}}}
...
\newcommand{\MerkleHashConvert}{\bitseq{\MerkleHashLengthConvert}}
...
\newcommand{\Convert}{\mathsf{ZKConvert}}
...
\newcommand{\ProofConvert}{\pi_\Convert}
...
\newcommand{\vAllowedConversion}[1]{\mathsf{v}_{#1}^\mathsf{convert}}
...
\newcommand{\convert}[1]{\texorpdfstring{{\color{brown}{#1}}}{#1}}
...
%          $\cvNew{}$ &$:= \ValueCommit{\ValueCommitRandNew{}}(\ValueNew{})$ \\[1ex]$
	$\NoteValueBase$ &$:= \abstJ(\PRFvcgMASP{\NoteToken}) $\\[1ex]
          $\cvNew{}$ &$:= \scalarmult{\ValueNew{}\ParamJ{h}}{\NoteValueBase} + \scalarmult{\ValueCommitRandNew{}}{ \FindGroupJHash\Of{\ascii{MASP\_\_r\_},\ascii{r}}} $ \\[1ex]
...
\begin{itemize}
  ...
  \item \masp{Compute $\NoteAddressRand{} = \cmOld{}
                                    = \NoteCommitMASP{\NoteCommitRand}(\reprJ\Of{\DiversifiedTransmitBase},
                                                                          \reprJ\Of{\DiversifiedTransmitPublic},
                                                                          \vOld{},
                                                                           \reprJ\Of{\FindGroupJHash\Of{\ascii{MASP\_\_r\_},\ascii{r}}})$.}
...
\masp{$\cvOld{} = \scalarmult{\ValueOld{} \ParamJ{h} }{\NoteValueBase} + \scalarmult{\ValueCommitRandOld{}}{ \FindGroupJHash\Of{\ascii{MASP\_\_r\_},\ascii{r}}} $}
...
\masp{$\cvNew{} =  \scalarmult{\ValueNew{} \ParamJ{h}}{\NoteValueBase} + \scalarmult{\ValueCommitRandNew{}}{ \FindGroupJHash\Of{\ascii{MASP\_\_r\_},\ascii{r}}} $  }
...
\convert{
  \introsection
  \lsubsubsection{Convert}{convert}
  
  Ordinarily, a MASP transaction that does not shield or unshield assets must achieve a homomorphic 
  net value balance of 0. Since every asset type has a pseudorandomly derived asset generator, it is 
  not ordinarily feasible to achieve a net value balance of 0 for the transaction without each asset 
  type independently having a net value balance of 0. Therefore, intentional burning and minting of 
  assets typically requires a public ``turnstile'' where some collection of assets are unshielded, burned 
  or minted in a public transaction, and then reshielded. Since this turnstile publicly reveals asset 
  ...
  and does not verify the integrity of a transaction as a whole. To ensure that a transaction containing Spend 
  and Output descriptions does not violate the invariants of the shielded pool (such as the total 
  unspent balance of each asset in the pool) the value commitments are added homomorphically and this 
  homomorphic sum is opened to reveal the transaction has a net value balance of 0. When shielded assets are burned 
  or minted in a MASP transaction, the homomorphic net value balance must be nonzero, and offset by shielding 
  or unshielding a corresponding amount of each asset.
  
  Instead of requiring the homomorphic sum of Spend and Output value commitments to sum to 0, 
  burning and minting of assets can be enabled by allowing the homomorphic sum of Spend and Output 
  value commitments to sum to either 0 or a multiple of an allowed conversion ratio. For example, if 
  distinct assets A and B can be converted in a 1-1 ratio (meaning one unit of A can be burned to 
  mint one unit of B) then the Spend and Output value commitments may sum to a nonzero value.
  
  Let $A_1, A_2, \ldots, A_n$ be distinct asset types. An \textit{allowed conversion} is a list of tuples
   $\{(A_1, V_1), (A_2, V_2), \ldots (A_n, V_n)\}$ where $V_1, \ldots, V_n$ are signed integers.
  
  The \textit{asset generator} of an allowed conversion is defined to be: $vb = [V_1] vb_1 + \ldots + [V_n] vb_n$
  where $vb_i$ is the asset generator of asset $A_i$. 
  
  Each allowed conversion is committed to a Jubjub point using a binding Bowe-Hopwood commitment 
  of its asset generator (it is not necessary to be hiding). All allowed conversion commitments 
  are stored in a public Merkle tree, similar to the Note commitment tree. Since the contents of 
  this tree are entirely public, allowed conversions may be added, removed, or modified at any time.
  
  In order for an unbalanced transaction containing burns and mints to get a net value balance of zero, 
  one or more value commitments burning and minting assets must be added to the value balance. Similar 
  to how Spend and Output circuits check the validity of their respective value commitments, the Convert 
  circuit checks the validity and integrity of:
  
  \begin{enumerate}
  \item    There exists an allowed conversion commitment in the Merkle tree, and
  \item    The imbalance in the value commitment is a multiple of an allowed conversion's asset generator
  \end{enumerate}
  
  In particular, this informally requires a Convert circuit with public inputs:
  ...
  \begin{enumerate}
  \item Merkle Path validity: $path, pos$ is a valid Merkle path from $cm$ to $rt$.
  \item Allowed conversion commitment integrity: $cm$ opens to $repr(vb)$
  \item Value commitment integrity: $cv^{mint} = [8*v] vb + [rcv] R$ where $R$ is the value commitment randomness base
  \end{enumerate} 
  ...
  \begin{enumerate}
  \item Checking the Convert description includes a valid and current $rt$
  \item Checking the value commitment $cv^{in} + cv^{mint} - cv^{out}$ opens to 0
  \end{enumerate}
  
  Directionality of allowed conversions must be enforced as well. That is, $v$ must be a non-negative 
  64 bit integer. If negative values of $v$ are allowed (or equivalently, unbounded large values of 
  $v$ in the prime order scalar field of the Jubjub curve) then an allowed conversion could happen 
  in the reverse direction, burning the assets intended to be minted and vice versa. 
  
  It is also critical not to allow cycles. For example, if $\{(A_1, -1), (A_2, 2)\}$ and $\{(A_1, 1), (A_2, -1)\}$ 
  are allowed conversions, then an unlimited amount of $A_2$ may be minted from a nonzero amount of $A_1$. Since 
  
  It may theoretically be possible to implement similar mechanisms with only the existing Spend and Output 
  circuits. For example, a Merkle tree of many Notes could be created with asset generator $[-1] vb_1 + vb_2$ 
  and many different values, allowing anyone to Spend these public Notes, which will only balance if proper 
  amounts of asset type 1 are Spent and asset type 2 are Output.
  
  However, the Nullifier integrity check of the Spend circuit reveals the nullifier of each of these Notes. 
  This removes the privacy of the conversion as the public nullifier is linkable to the allowed conversion. 
  In addition, each Note has a fixed value, preventing arbitrary value conversions. In general, it 
  does not seem feasible to use only the Spend and Output circuits to implement the desired mechanisms.
  
  In principle, as long as the Merkle tree only contains allowed conversions, this should 
  permit the allowed conversions while maintaining other invariants. Note that since the asset 
  generators are not derived in the circuit, all sequences of values and asset types are allowed. 
  
     \lsubsubsection{\ConvertStatement{}}{convertstatement}

The new MASP Convert circuit has 47358 constraints. 

\vspace{-1ex}
Let $\ScalarLength$, $\ValueCommitAlg$, $\NoteCommitSaplingAlg$, $\GroupJ$, $\SubgroupJ$, $\reprJ$, $\ParamJ{q}$, $\ParamJ{r}$, $\ParamJ{h}$, \\ 
$\ExtractJ \typecolon \SubgroupJ \rightarrow \MerkleHashSapling$ ,  $\AuthProveBase$ be as defined in the original \Sapling specification.

Let $\MerkleDepthConvert=\MerkleDepthSapling$ and $\MerkleHashLengthConvert = \MerkleHashLengthSapling$.

\intropart
\vspace{0.5ex}
A valid instance of $\ProofConvert$ assures that given a \primaryInput:

\vspace{-1ex}
\begin{formulae}
  \item $\oparen\rt \typecolon \MerkleHashConvert,\\
         \hparen\cvAllowedConversion{} \typecolon \ValueCommitConvert\cparen$
\end{formulae}
...
\vspace{-1ex}
\begin{formulae}
  \item $\oparen\TreePath{} \typecolon \typeexp{\MerkleHash}{\MerkleDepthConvert},\\
         \hparen\NotePosition \typecolon \NotePositionTypeConvert,\vspace{0.4ex}\\
         \hparen\vAllowedConversion{} \typecolon \ValueType,\\
         \hparen\ValueCommitRandConvert{} \typecolon \binaryrange{\ScalarLength},\\
         \hparen\cmAllowedConversion{} \typecolon \GroupJ,\\
         \hparen\AllowedConversionValueBase \typecolon \GroupJ\cparen$ %masp
\end{formulae}
\vspace{-1.5ex}
such that the following conditions hold:

\vspace{0.5ex}
\snarkcondition{AllowedConversion commitment integrity}{allowedconversioncommitmentintegrity}
$\cmAllowedConversion{} = \mathsf{PedersenHashToPoint}(''MASP\_\_PH'', [1]^6||\reprJ\Of{\AllowedConversionValueBase})$.%masp

\snarkcondition{Merkle path validity}{spendmerklepathvalidity}
Either $\vAllowedConversion{} = 0$; or $(\TreePath{}, \NotePosition)$ is a valid \merklePath of depth $\MerkleDepthConvert$, from $\cmU = \ExtractJ(\cmAllowedConversion{})$ to the \anchor $\rt$.

\snarkcondition{Value commitment integrity}{spendvaluecommitmentintegrity}
%$\cvOld{} = \ValueCommit{\ValueCommitRandOld{}}(\vOld{})$.
$\cvAllowedConversion{} = \scalarmult{\ValueConvert{} \ParamJ{h} }{\AllowedConversionValueBase} + \scalarmult{\ValueCommitRandConvert{}}{ \FindGroupJHash\Of{\ascii{MASP\_\_r\_},\ascii{r}}} $

\snarkcondition{Small order checks}{spendnonsmall}
$\AllowedConversionValueBase$ is not of small order, i.e.\ $\scalarmult{\ParamJ{h}}{\AllowedConversionValueBase} \neq \ZeroJ$

The form and encoding of \convertStatement proofs may be Groth16.
...
\begin{pnotes}
  \item Public and \auxiliaryInputs{} \MUST be constrained to have the types specified. In particular,
        ...
        The $\ValueCommitConvert$ type also represent points, i.e. $\GroupJ$.
  \item In the Merkle path validity check, each \merkleLayer does \emph{not} check that its
        input bit sequence is a canonical encoding (in $\range{0}{\ParamS{r}-1}$) of the integer
        from the previous \merkleLayer.
  \item It is \emph{not} checked that $\ValueCommitRandConvert{} < \ParamJ{r}$.
        \item 
        Note that the asset identifiers are \textit{not} witnessed in the \convertStatement. 
        Since $\AllowedConversionValueBase$ is publicly computed (or is otherwise trusted), the \convertStatement
        assumes that $\AllowedConversionValueBase$ provides a correct allowed conversion.
\end{pnotes}
}
