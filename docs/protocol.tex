...
Let $\ValueCommitAlg$, $\KASapling$, $\DiversifyHash$, $\reprJ$, $\ParamJ{r}$, and $\ParamJ{h}$ be as defined in the original \Sapling specification.
...
\masp{
  ...
  represent the bit-decomposition of an element in the scalar field of an elliptic curve with order $r_\GroupJ$. 
  Unlike the variables $rcm, rcv$, the specification does not mention explicitly that the corresponding field 
  elements do not need to constrained to the range $\{0, \ldots , r_\GroupJ - 1\}$. This would require more 
